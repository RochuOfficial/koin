...
1. **Verify Logic Porting**
   - Ensure `src/lib/store.ts` (web) logic is fully captured in `koin-mobile/src/lib/store.ts` (mobile), specifically the XP calculation and goal tracking logic.
   - Confirm that the AI Coach webhook URL and prompt logic are consistent.

2. **Asset Inventory**
   - Identify any unique images or icons in `public/` or `src/assets/` that are not yet in `koin-mobile/assets/`.
   - List custom fonts (if any) currently used in the web version to be registered in `koin-mobile/app/_layout.tsx`.
